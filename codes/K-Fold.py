...
def k_fold(foldNum):
    ...
    print("K-Fold iteration: " , foldNum)
...
for i in range(1, 11):
    ...
    testCat = []
    for j in range(4000):
        if j not in range(startIndex, endIndex):
            trainText.append(text[j])
            trainCat.append(cat[j])
    testText = text[startIndex:endIndex]
    testCat= cat[startIndex:endIndex]
    k_fold(i)
    startIndex+=400
    endIndex+=400
